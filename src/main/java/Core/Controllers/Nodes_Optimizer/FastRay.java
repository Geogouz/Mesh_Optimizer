...
class FastRay {
    ...
    FastRay(int[] parent_refractions,
            double node_origin_x, double node_origin_y, double max_destination_x, double max_destination_y,
            double[] vector, int reflections_occurred, Integer obstruction_id, double traversed_distance) {
        //super(node_origin_x, node_origin_y, max_destination_x, max_destination_y);
        start_x = node_origin_x;
        start_y = node_origin_y;
        end_x = max_destination_x;
        end_y = max_destination_y;
    ...
    boolean reaches_cell(Cell sample_cell) {
        ...
        double[] intersection_point_with_diagonal_1 = lineSegmentLineSegmentIntersection(start_x, start_y, end_x, end_y,
                sample_cell.diagonal_1[0], sample_cell.diagonal_1[1], sample_cell.diagonal_1[2], sample_cell.diagonal_1[3]);
        ...
        // If it does not, check whether this Ray intersects with the second diagonal of the sample point
            ...
            double[] intersection_point_with_diagonal_2 =
                    lineSegmentLineSegmentIntersection(start_x, start_y, end_x, end_y, sample_cell.diagonal_2[0],
                            sample_cell.diagonal_2[1], sample_cell.diagonal_2[2], sample_cell.diagonal_2[3]);
            return intersection_point_with_diagonal_2 != null;
        }
    }

    double[] getRayIntersection(Line obstruction) {
        double[] points = lineSegmentLineSegmentIntersection(start_x, start_y, end_x, end_y,
                obstruction.getStartX(), obstruction.getStartY(), obstruction.getEndX(), obstruction.getEndY());
        if (points != null) {
    ...
    FastRay get_reflected_ray(int[] parent_refractions, Map.Entry<Integer, double[]> obstruction_entry, double[] obstruction_normal) {
        ...
        double[] new_vector = subtraction_of_this_ray_vector_with_another_vector(ray_vector, vector_scalar_multiplication(obstruction_normal, 2 * dot_product_of_this_ray_vector_with_another_vector(ray_vector, obstruction_normal)));
        ...
        int new_reflections_already_occurred = reflections_already_occurred;
        new_reflections_already_occurred = new_reflections_already_occurred + 1;

        return new FastRay(parent_refractions, obstruction_entry.getValue()[0], obstruction_entry.getValue()[1], destination_point[0],
